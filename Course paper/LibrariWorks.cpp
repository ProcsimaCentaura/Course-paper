...
//Функции поиска по библиотеке, будут сначала сортировать библитеку(если она еще не отсортирована по этому типу) после чего проводить в ней бинарный поиск.
//Поиск по имени
int searchByName(Library* library, const char* name) {
	if (library->sort.field != FieldsOfBooks::name) {
		sortLibrary(library, FieldsOfBooks::name, library->sort.type);
	}
	Book key;
...
int searchByAutor(Library* library, const char* autor) {
	if (library->sort.field != FieldsOfBooks::autor) {
		sortLibrary(library, FieldsOfBooks::autor, library->sort.type);
	}
	Book key;
...
int searchByGenre(Library* library, const char* genre) {
	if (library->sort.field != FieldsOfBooks::genre) {
		sortLibrary(library, FieldsOfBooks::genre, library->sort.type);
	}
	Book key;
